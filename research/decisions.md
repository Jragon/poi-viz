...
The Rastaxel follow-up treats a full motif as two units: one pendulum cycle followed by one circle. Each track is compiled as eight explicit quarter-unit segments; each segment stores its boundary pose and a lab runtime callback for only that slice. Each hand has an independent inward/outward flow control, resolved through the beat-graph handedness convention (left inward/right inward are clockwise/counterclockwise respectively). The resolved phase direction is shared by that hand's pendulum swings and circle handoff, so signed velocity direction is preserved at the join. The right-hand offset selects the corresponding phase-shifted slice in whole steps (0–7), while the motion remains continuous at segment boundaries. The graph reports both tracks' instantaneous speed in circle-equivalents and makes the raw pendulum-bottom-to-circle handoff visible, including its speed discontinuity. This is an experiment for calibrating timing and gravity-derived speed; it is not a new engine driver law.
