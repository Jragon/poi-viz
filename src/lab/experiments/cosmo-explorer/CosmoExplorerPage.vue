...
import { REEL_POSITION_LABELS, resolveDirections } from "@/lab/experiments/reel-explorer/reelRules";
import type { ReelDirection } from "@/lab/experiments/reel-explorer/types";
...
const config = ref<CosmoConfig>(DEFAULT_COSMO_CONFIG);
...
const graph = computed(() => buildCosmoBeatGraph(config.value));
...
const cosmoSummaryLabel = computed(
  () =>
...
function setBackPosition(hand: PoiBeatHand, position: CosmoBackPosition): void {
  ...
  config.value = { ...config.value, [hand]: { ...currentPair, b: position } };
}
...
function formatPair(pair: CosmoPositionPair): string {
...
function frontButtonClass(hand: PoiBeatHand, position: CosmoFrontPosition): string {
  if (config.value[hand].a === position) return "border-slate-200 bg-slate-100 text-slate-950";
...
function backButtonClass(hand: PoiBeatHand, position: CosmoBackPosition): string {
  ...
  if (!isValidCosmoPair(config.value[hand].a, position)) {
  ...
  <main class="min-h-screen bg-slate-950 px-5 py-8 text-slate-100 md:px-8 md:py-10">
    <section class="mx-auto grid max-w-7xl gap-5 lg:grid-cols-[minmax(18rem,24rem)_minmax(0,1fr)]">
      <div class="grid content-start gap-4">
        <header>
          <p class="text-xs uppercase tracking-[0.2em] text-slate-500">Cosmo Explorer</p>
          <h1 class="mt-2 text-2xl font-semibold text-slate-50">Generated cosmo graph</h1>
        </header>
                ...
                <div class="grid grid-cols-[1.25rem_minmax(0,1fr)] items-start gap-2">
                  ...
                  <div class="grid grid-cols-2 gap-1.5">
                    <button
                      ...
                      :disabled="!isValidCosmoPair(config[hand].a, position)"
        ...
        <section class="rounded-lg border border-slate-800 bg-slate-900/60 px-4 py-3">
          <p class="text-xs uppercase tracking-[0.16em] text-slate-500">Resolved cosmo</p>
        ...
        <section class="overflow-hidden rounded-lg border border-slate-800 bg-slate-950/80">
          ...
          >
            <div class="min-w-0">
              ...
              <h2 class="mt-1 text-lg font-semibold text-slate-100">Compiled cosmo</h2>
            </div>
