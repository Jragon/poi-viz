...
function parseFixtureCase(candidate: unknown, defaults: AppState): FixtureCaseDefinition {
  ...
  const hydratedState = deserializeState(
    JSON.stringify({
  ...
  if (!hydratedState) {
    throw new Error(`Fixture case "${id}" has an invalid AppState payload.`);
  }

  return {
    id,
    state: cloneState(hydratedState)
  };
}
 ...
 * @param defaults Default state used for strict state-shape validation.
